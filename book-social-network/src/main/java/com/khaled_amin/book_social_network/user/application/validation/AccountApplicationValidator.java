...
public class AccountApplicationValidator {
    ...
    public void validateCreate(AccountCreateCommand command, List<Long> roleIds, List<Role> fetchedRoles) {

        if (command == null) {
            throw AccountApplicationException
                    .invalidCommand()
                    .withDetail("reason", "CreateAccountCommand must not be null");
        }

        ensureUsernameUnique(command.username().value());
        ensureEmailUnique(command.email().value());

        validateAccountRoles(roleIds, fetchedRoles);
    }
    ...
    public void validateUpdate(Account account, AccountUpdateCommand command) {
        ...
        if (command == null) {
            throw AccountApplicationException
                    .invalidCommand()
                    .withDetail("reason", "UpdateAccountCommand must not be null");
        }
