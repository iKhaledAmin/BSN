...
public class AccountServiceImpl implements AccountService {
    ...
    public Account register(AccountCreateCommand command) {
        List<Long> defaultRoleIds = roleService.getAllDefaultRoleIds();
        return create(command, defaultRoleIds);
    }


    @Transactional
    @Override
    public Account create(AccountCreateCommand command, List<Long> roleIds) {
        ...
        List<Role> fetchedRoles = resolveAccountRoles(normalizedIds);

        // Application validation
        accountValidator.validateCreate(command, normalizedIds, fetchedRoles);

        // Policy validation
        accountPolicyEngine.canCreate(
                policyContextFactory.forCreate(actor, fetchedRoles)
        );

        // Domain logic
        Account account = accountFactory.create(command, fetchedRoles);
    ...
    private List<Role> resolveAccountRoles(List<Long> roleIds) {

            // If roles explicitly provided → use them
            if (!roleIds.isEmpty()) {
                return roleService.getAllByIds(roleIds);
            }

            // Otherwise → fallback to default roles
